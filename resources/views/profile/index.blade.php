<x-layouts.app title="Profil & Dashboard">
    <div class="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div class="mb-6 sm:mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            <div>
            ...
            <div class="flex flex-wrap items-center gap-2.5 sm:gap-3">
                <a
                    ...
                    class="inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-700 hover:bg-emerald-800 px-4 py-2.5 sm:px-5 text-sm font-bold text-white shadow-md transition hover:shadow-lg"
                >
                ...
                <form id="logout-user-form" method="POST" action="{{ route('logout') }}">
                    ...
                    <button
                        ...
                        class="inline-flex items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-2.5 sm:px-5 text-sm font-bold text-red-600 transition hover:bg-red-100 hover:text-red-700 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-400 cursor-pointer"
                    >
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            ...
            {{-- Profile Card --}}
            <div class="rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-6 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                ...
                <div class="mt-6 border-t border-slate-100 pt-5 space-y-3 text-sm dark:border-slate-800">
                    @if(auth()->user()->class_room || auth()->user()->role === 'student')
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-xs font-medium text-slate-400 shrink-0">Kelas / Rombel</span>
                            <span class="font-bold text-emerald-700 dark:text-emerald-400 text-xs sm:text-sm truncate text-right">{{ auth()->user()->class_room ?? '-' }}</span>
                        </div>
                    @endif

                    <div class="flex items-center justify-between gap-2">
                        <span class="text-xs font-medium text-slate-400 shrink-0">Email</span>
                        <span class="font-bold text-slate-800 dark:text-slate-200 text-xs sm:text-sm truncate text-right">{{ auth()->user()->email ?? '-' }}</span>
                    </div>

                    <div class="flex items-center justify-between gap-2">
                        <span class="text-xs font-medium text-slate-400 shrink-0">No. Telp</span>
                        <span class="font-bold text-slate-800 dark:text-slate-200 text-xs sm:text-sm truncate text-right">{{ auth()->user()->phone ?? '-' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 grid grid-cols-2 gap-3 sm:gap-4">
                ...
                <div class="flex flex-col justify-between rounded-3xl border border-slate-200/80 bg-white p-4 sm:p-6 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-2xl bg-emerald-50 text-lg sm:text-xl text-emerald-700 border border-emerald-100">
                        <i class="fa-solid fa-bag-shopping"></i>
                    </div>
                    <div class="mt-3 sm:mt-4">
                        <p class="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-400">Total Pesanan</p>
                        <p class="mt-1 text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">{{ $totalOrders ?? 0 }}</p>
                    </div>
                </div>

                {{-- Pending Orders --}}
                <div class="flex flex-col justify-between rounded-3xl border border-slate-200/80 bg-white p-4 sm:p-6 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-2xl bg-amber-50 text-lg sm:text-xl text-amber-600 border border-amber-100 dark:bg-amber-950/40">
                        <i class="fa-solid fa-clock font-bold"></i>
                    </div>
                    <div class="mt-3 sm:mt-4">
                        <p class="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-400 leading-tight">Menunggu Verifikasi</p>
                        <p class="mt-1 text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">{{ $pendingOrders ?? 0 }}</p>
                    </div>
                </div>
                ...
                <div class="flex flex-col justify-between rounded-3xl border border-slate-200/80 bg-white p-4 sm:p-6 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <div class="mt-3 sm:mt-4">
                        <p class="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-400">Pesanan Selesai</p>
                        <p class="mt-1 text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">{{ $completedOrders ?? 0 }}</p>
                    </div>
                </div>
                ...
                <div class="flex flex-col justify-between rounded-3xl border border-slate-200/80 bg-white p-4 sm:p-6 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <div class="mt-3 sm:mt-4">
                        <p class="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-slate-400">Item Keranjang</p>
                        <p class="mt-1 text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">{{ $cartCount ?? 0 }}</p>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6 sm:mt-8">
            ...
            <div class="grid grid-cols-1 gap-3.5 sm:gap-4 sm:grid-cols-3">
        ...
        <div class="mt-6 sm:mt-8">
            @if (auth()->user()->seller?->status === 'approved')
                ...
                <div class="flex flex-col justify-between gap-5 rounded-3xl border border-emerald-200 bg-emerald-50/60 p-5 sm:p-8 shadow-xs dark:border-emerald-950 dark:bg-emerald-950/20 sm:flex-row sm:items-center">
                    <div>
                        ...
                        <h2 class="mt-2.5 text-lg font-black text-slate-900 dark:text-white sm:text-2xl">
                    ...
                    <a
                        ...
                        class="inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-700 px-5 py-3 sm:px-6 sm:py-3.5 text-sm font-bold text-white shadow-xs transition hover:bg-emerald-800 shrink-0 w-full sm:w-auto"
                    >
            ...
            @elseif (auth()->user()->seller?->status === 'pending')
                ...
                <div class="rounded-3xl border border-amber-200 bg-amber-50/60 p-5 sm:p-8 shadow-xs dark:border-amber-950 dark:bg-amber-950/20">
                    <div class="flex items-start sm:items-center gap-3.5 sm:gap-4">
                        <div class="flex h-10 w-10 sm:h-12 sm:w-12 shrink-0 items-center justify-center rounded-2xl bg-amber-100 text-amber-800 text-lg sm:text-xl font-bold dark:bg-amber-900 dark:text-amber-200">
                            <i class="fa-solid fa-clock"></i>
                        </div>
                        <div>
                            <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-2.5 py-0.5 text-[11px] sm:text-xs font-bold text-amber-800 dark:bg-amber-900 dark:text-amber-200">
                                Status: Menunggu Verifikasi Admin
                            </span>
                            <h2 class="mt-1 text-base font-black text-slate-900 dark:text-white sm:text-xl">
            ...
            @elseif (auth()->user()->seller?->status === 'revision')
                ...
                <div class="flex flex-col justify-between gap-5 rounded-3xl border border-amber-300 bg-amber-50 p-5 sm:p-8 shadow-xs dark:border-amber-900/80 dark:bg-amber-950/30 sm:flex-row sm:items-center">
                    <div>
                        ...
                        <p class="mt-1 text-xs sm:text-sm font-semibold text-amber-900 dark:text-amber-300">
                    ...
                    <a
                        ...
                        class="inline-flex items-center justify-center gap-2 rounded-2xl bg-amber-600 px-5 py-3 sm:px-6 sm:py-3.5 text-sm font-bold text-white shadow-md shadow-amber-600/20 transition hover:bg-amber-700 shrink-0 w-full sm:w-auto"
                    >
            ...
            @elseif (auth()->user()->seller?->status === 'rejected')
                ...
                <div class="flex flex-col justify-between gap-5 rounded-3xl border border-rose-200 bg-rose-50/70 p-5 sm:p-8 shadow-xs dark:border-rose-900/60 dark:bg-rose-950/20 sm:flex-row sm:items-center">
                    <div>
                        ...
                        <p class="mt-1 text-xs sm:text-sm font-semibold text-rose-800 dark:text-rose-300">
                    ...
                    <a
                        href="{{ route('buyer.apply-seller') }}"
                        class="inline-flex items-center justify-center gap-2 rounded-2xl bg-rose-600 px-5 py-3 sm:px-6 sm:py-3.5 text-sm font-bold text-white shadow-md shadow-rose-600/20 transition hover:bg-rose-700 shrink-0 w-full sm:w-auto"
                    >
            ...
            @else
                ...
                <div class="flex flex-col justify-between gap-5 rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-8 shadow-xs dark:border-slate-800 dark:bg-slate-900 sm:flex-row sm:items-center">
                    <div>
        ...
        <div class="mt-6 sm:mt-8 rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-6 shadow-xs dark:border-slate-800 dark:bg-slate-900">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
                <div class="pr-2">
                    <h3 class="font-bold text-slate-900 dark:text-white flex items-center gap-2 text-base sm:text-lg">
                ...
                <a href="{{ route('profile.activity-logs') }}" class="text-xs font-bold text-emerald-700 hover:underline dark:text-emerald-400 shrink-0 inline-flex items-center gap-1 whitespace-nowrap self-start sm:self-auto pt-1 sm:pt-0">
                ...
                @forelse ($recentActivityLogs ?? [] as $log)
                    <div class="py-3 flex items-center justify-between gap-3">
                        ...
                        <div class="text-right shrink-0">
                            <span class="text-[11px] font-semibold text-slate-500 dark:text-slate-400 whitespace-nowrap">
