<x-layouts.buyer title="Detail Pesanan">
    ...
    <div class="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        @else
            <div class="mb-8 rounded-3xl border border-red-200 bg-red-50 p-5 text-center dark:border-red-900/60 dark:bg-red-950/40">
                <p class="text-sm font-bold text-red-700 dark:text-red-400"><i class="fa-solid fa-circle-xmark mr-1"></i> Pesanan Ini Telah Dibatalkan / Ditolak oleh Penjual</p>
            </div>
        @endif
                ...
                @if (strtolower($order->payment?->method ?? '') === 'qris')
                    <div class="rounded-3xl border border-emerald-200 bg-emerald-50/60 p-6 shadow-xs dark:border-emerald-900/60 dark:bg-emerald-950/30">
                        ...
                        @if ($order->seller?->qris_image)
                            <div class="mt-4 flex flex-col items-center justify-center rounded-2xl bg-white p-5 border border-emerald-100 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                                <img
                                    src="{{ Storage::url($order->seller->qris_image) }}"
                                    alt="QRIS Toko {{ $order->seller->user?->username }}"
                                    class="max-h-72 w-auto rounded-xl object-contain border border-slate-100 dark:border-slate-800"
                                >
                                <p class="mt-3 text-xs font-bold text-slate-800 dark:text-slate-200">
                        ...
                        <div class="mt-5 border-t border-emerald-200/80 pt-4 dark:border-emerald-900/60">
                            ...
                            @if($order->payment?->proof)
                                <div class="mt-3 rounded-2xl bg-white p-4 border border-slate-200 flex flex-col sm:flex-row items-center gap-4 dark:border-slate-800 dark:bg-slate-900">
                                    <img
                                        src="{{ Storage::url($order->payment->proof) }}"
                                        alt="Bukti Pembayaran"
                                        class="h-24 w-24 rounded-xl border border-slate-200 object-cover shadow-xs cursor-pointer dark:border-slate-700"
