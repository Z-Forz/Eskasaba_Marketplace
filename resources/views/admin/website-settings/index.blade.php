<x-layouts.admin title="Pengaturan Website">
    <div class="mx-auto max-w-5xl space-y-6">
        ...
        >
            ...
            {{-- Callback URLs Box --}}
            <div class="rounded-3xl border border-emerald-200/80 bg-emerald-50/40 p-6 shadow-xs dark:border-emerald-900/50 dark:bg-emerald-950/20">

                <h2 class="text-lg font-bold text-emerald-950 dark:text-emerald-300 flex items-center gap-2">
                    <i class="fa-solid fa-link text-emerald-600"></i> Link Callback & Integration Endpoints
                </h2>

                <p class="mt-1 text-xs text-slate-600 dark:text-slate-400">
                    Gunakan tautan callback di bawah ini untuk mengonfigurasi Webhook Payment Gateway (Midtrans, Tripay, QRIS), SSO API Sekolah, atau Bot WhatsApp.
                </p>

                <div class="mt-4 space-y-3 text-xs">
                    <div>
                        <label class="block font-bold text-slate-700 dark:text-slate-300 mb-1">
                            💳 Link Callback Pembayaran (Payment Gateway / QRIS Webhook)
                        </label>
                        <div class="flex items-center gap-2">
                            <input
                                type="text"
                                readonly
                                value="{{ url('/api/payment/callback') }}"
                                class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 font-mono text-xs text-emerald-700 shadow-2xs dark:border-slate-700 dark:bg-slate-900 dark:text-emerald-400"
                            >
                            <button
                                type="button"
                                onclick="navigator.clipboard.writeText('{{ url('/api/payment/callback') }}'); alert('Callback Link Pembayaran berhasil disalin!');"
                                class="shrink-0 rounded-xl bg-emerald-700 px-3 py-2 text-xs font-bold text-white hover:bg-emerald-800 transition"
                            >
                    ...
                    <div>
                        <label class="block font-bold text-slate-700 dark:text-slate-300 mb-1">
                            🎓 Link Callback SSO / Database API Sekolah
                        </label>
                        <div class="flex items-center gap-2">
                            <input
                                type="text"
                                readonly
                                value="{{ url('/auth/school/callback') }}"
                                class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 font-mono text-xs text-emerald-700 shadow-2xs dark:border-slate-700 dark:bg-slate-900 dark:text-emerald-400"
                            >
                            <button
                                type="button"
                                onclick="navigator.clipboard.writeText('{{ url('/auth/school/callback') }}'); alert('Callback Link Sekolah berhasil disalin!');"
                                class="shrink-0 rounded-xl bg-emerald-700 px-3 py-2 text-xs font-bold text-white hover:bg-emerald-800 transition"
                            >
                                <i class="fa-regular fa-copy"></i> Salin
                            </button>
                    ...
                    <div>
                        <label class="block font-bold text-slate-700 dark:text-slate-300 mb-1">
                            📱 Endpoint Bot WhatsApp Gateway (Local Node API)
                        </label>
                        <div class="flex items-center gap-2">
                            <input
                                type="text"
                                readonly
                                value="{{ config('services.whatsapp.url', 'http://localhost:3000/send-message') }}"
                                class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 font-mono text-xs text-slate-700 shadow-2xs dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"
                            >
                        </div>
                    </div>
                </div>
