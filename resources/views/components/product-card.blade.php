...
>
    ...
    <div class="relative aspect-4/3 w-full overflow-hidden bg-slate-100 dark:bg-slate-800/80">
        ...
        <a href="{{ route('products.show', $product) }}" class="block h-full w-full">
            @if($firstImage)
                ...
                >
            @else
                <div class="flex h-full w-full flex-col items-center justify-center bg-linear-to-br from-emerald-50 via-slate-50 to-slate-100 text-slate-400 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 dark:text-slate-500">
        ...
        {{-- Badges Overlay --}}
        <div class="pointer-events-none absolute inset-x-3 top-3 flex items-start justify-between gap-2 z-10">

            {{-- Category & Stock Status Badges --}}
            <div class="flex flex-wrap gap-1.5">
                @if(count($flavors) > 0)
                    <span class="rounded-full bg-emerald-950/85 px-3 py-1 text-[10px] font-extrabold tracking-wide text-emerald-300 shadow-md backdrop-blur-md border border-emerald-500/30 flex items-center gap-1">
                        <i class="fa-solid fa-utensils text-[9px] text-emerald-400"></i> {{ count($flavors) }} Varian
                    </span>
                @endif
                ...
                @else
                    <span class="rounded-full bg-emerald-700/90 px-3 py-1 text-[10px] font-extrabold tracking-wide text-white shadow-md backdrop-blur-md border border-emerald-400/30">
                        Stok: {{ $stock }}
                    </span>
                @endif
            </div>
            ...
            {{-- Discount Badge --}}
            @if($hasDiscount)
                <span class="rounded-full bg-gradient-to-r from-red-600 to-rose-600 px-3 py-1 text-[10px] font-black tracking-wide text-white shadow-md backdrop-blur-md border border-white/20">
    ...
    <div class="flex flex-1 flex-col p-4 sm:p-5">
        ...
        <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-1 text-[11px] font-bold text-amber-500 bg-amber-50 px-2 py-0.5 rounded-lg border border-amber-200/50 dark:bg-amber-950/40 dark:border-amber-900/40">
                <i class="fa-solid fa-star text-[10px]"></i>
                <span>{{ number_format($product->reviews_avg_rating ?? 4.8, 1) }}</span>
            </div>
        </div>
