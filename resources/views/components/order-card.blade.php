...
@php
    ...
    $firstProductName = $firstItem?->product_name ?: ($firstItem?->product?->name ?: 'Produk Pesanan');
    $opt = $firstItem?->variant_name ?: $firstItem?->note;
    $qty = $firstItem?->quantity ?? 1;
    $itemSubTitle = !empty($opt) ? "{$opt} / {$qty} Pcs" : "{$qty} Pcs";
    $itemCount = $order->items?->count() ?? 1;

    $orderTitle = $itemCount > 1
        ? $firstProductName . ' (+' . ($itemCount - 1) . ' produk lain)'
        : $firstProductName;
@endphp
...
<div class="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-xs transition hover:border-slate-300 dark:border-slate-800 dark:bg-slate-900 sm:p-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div class="min-w-0 flex-1">
            ...
            <h3 class="truncate text-base font-black text-slate-900 dark:text-white sm:text-lg" title="{{ $orderTitle }}">
                {{ $orderTitle }}
            </h3>

            <p class="mt-0.5 text-xs font-bold text-emerald-700 dark:text-emerald-400 flex items-center gap-1">
                <i class="fa-solid fa-layer-group text-[10px]"></i> {{ $itemSubTitle }}
            </p>
        </div>
