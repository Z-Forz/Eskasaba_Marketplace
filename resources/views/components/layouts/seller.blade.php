...
<head>
    ...
    <title>{{ $title ?? 'Panel Seller' }} - {{ $settings->website_name ?? config('app.name', 'Marketplace') }}</title>
...
<body class="flex min-h-full flex-col bg-slate-100 font-['Plus_Jakarta_Sans',sans-serif] text-slate-800 antialiased dark:bg-slate-950 dark:text-slate-200">
    ...
    <div class="flex flex-1 flex-col lg:pl-56">
        ...
        <header class="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 dark:border-slate-800 dark:bg-slate-900/90 sm:px-6 backdrop-blur-[2px]">
            <div class="flex items-center gap-3">
                {{-- Mobile Hamburger Toggle Button Seller --}}
                <button
                    type="button"
                    class="seller-mobile-sidebar-toggle-btn inline-flex items-center justify-center rounded-xl border border-slate-200 bg-slate-100 p-2 text-slate-700 hover:bg-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 lg:hidden cursor-pointer"
    ...
    @if(session('success') || session('error') || session('warning') || session('info'))
        <div class="fixed bottom-5 left-4 right-4 sm:left-auto sm:right-5 sm:w-full sm:max-w-md z-[70] space-y-2 pointer-events-none">
            @if(session('success'))
